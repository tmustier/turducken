...
WORKDIR="${TURDUCKEN_WORKDIR:-$ROOT_DIR/.workdir}"
WTERM_DIR="${TURDUCKEN_WTERM_DIR:-$WORKDIR/wterm-$UPSTREAM_SHORT}"
HOST="${TURDUCKEN_HOST:-127.0.0.1}"
PORT="${TURDUCKEN_PORT:-3001}"
...
BACKEND_SESSION="${TURDUCKEN_BACKEND_SESSION:-cursed-backend}"
CARBONYL_SESSION="${TURDUCKEN_CARBONYL_SESSION:-cursed-carbonyl}"
PI_SESSION="${TURDUCKEN_PI_SESSION:-cursed-pi}"
PI_CWD="${TURDUCKEN_PI_CWD:-$HOME}"
PI_CMD="${TURDUCKEN_PI_CMD:-pi}"
URL="http://$HOST:$PORT"
...
echo
echo "Build the interactive bridge:"
echo "  $ROOT_DIR/scripts/use-the-curse.sh"
